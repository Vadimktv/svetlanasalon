          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-16">
            {[
              { 
                ...
                desc: 'Честные рекомендации по уходу и подбору образа.',
              ...
              { 
                title: 'Без суеты и ожиданий', 
                desc: 'Удобная онлайн-запись, где администратор всё подтверждает лично.',
                icon: <svg fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              },
              { 
                title: 'Сервис высшего класса', 
                desc: 'Чай, кофе и забота в каждой детали визита.',
                icon: <svg fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" /></svg>
              }
            ].map((item, i) => (
              <div key={i} className="flex gap-6 items-start bg-white/5 border border-white/10 p-8 rounded-3xl hover:bg-white/10 transition-colors shadow-lg">
                <div className="min-w-[48px] h-[48px] bg-[#ebdcd4]/20 rounded-full flex items-center justify-center shrink-0 border border-[#ebdcd4]/30 text-[#ebdcd4]">
                  {item.icon}
                </div>
                <div>
