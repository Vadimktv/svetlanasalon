...
export function BookingModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm flex items-end md:items-center justify-center p-0 md:p-4 transition-opacity duration-300">
      <div className="bg-[#fffdf9]/95 backdrop-blur-md w-full max-w-[440px] rounded-t-[40px] md:rounded-[40px] p-8 pb-12 md:pb-8 shadow-2xl relative animate-slide-up md:animate-fade-in border border-[#dfcaa4]/50">
        <button 
          onClick={onClose} 
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xs text-[#8d6533] uppercase tracking-[0.18em]">Выберите услугу и удобное время</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <input type="date" name="date" aria-label="Желаемая дата" className="w-full bg-[#f8f3eb] border-0 rounded-2xl px-4 py-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#af8750]/40 transition-all text-[#2c2c2c]" />
            <input type="time" name="time" aria-label="Желаемое время" className="w-full bg-[#f8f3eb] border-0 rounded-2xl px-4 py-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#af8750]/40 transition-all text-[#2c2c2c]" />
          </div>
          ...
          <button type="submit" className="w-full gold-button text-white py-4 rounded-full font-bold tracking-widest text-xs uppercase hover:-translate-y-0.5 transition-all shadow-lg mt-2">
